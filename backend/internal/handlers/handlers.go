...
func New(cfg config.Config, log *slog.Logger, authSvc *auth.Service, contacts store.ContactsRepo, ethSvc *eth.RPCService, pg *postgres.Provider, watcher *nftscan.Watcher, scanner *scan.TxScanner) *Handlers {
	return &Handlers{
		Cfg:        cfg,
		Log:        log,
		Auth:       authSvc,
